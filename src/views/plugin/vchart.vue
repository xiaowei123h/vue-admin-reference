...
onMounted(() => {
  ...
  // @ts-ignore
  const vchart = new VChart(spec, { dom: 'vchart' });
  vchart.renderSync();
  // #endregion
  ...
  function logScale(value: any, domain: any, range: any) {
    // 计算域和范围的对数
    const logDomain = domain.map((x: any )=> (x !== 0 ? Math.log10(x) : 0));
    const logRange = range.map((x: any) => Math.log10(x));
    ...
    series: [
      {
        ...
        size: (d: any) => logScale(d.Population, [0, Math.max(...data2.map(d => d.Population))], [1, 20])
      }
    ],
    crosshair: {
      yField: {
    ...
    axes: [
      {
        ...
        type: 'linear',
        range: {
          min: Math.min(...data2.map(d => d.LifeExpectancy)),
          max: Math.max(...data2.map(d => d.LifeExpectancy))
        },
        title: {
          visible: true,
          text: 'LifeExpectancy'
  ...
  // @ts-ignore
  const vchart2 = new VChart(spec2, { dom: 'vchart2' });
  vchart2.renderSync();
  // #endregion
})
</script>
...
<template>
  <div class="p-16">
    <el-card class="card-w">
      <div id="vchart" class="w-100% h-454px"></div>
    </el-card>
    <el-card class="card-w m-t-20">
      <div id="vchart2" class="w-100% h-454px"></div>
    </el-card>
  </div>
</template>